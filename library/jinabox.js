...
class Floater extends HTMLElement {
    constructor() {
        ...
        if (this.settings.theme !== 'default')
            this.closeIcon = embedIcons.closeLight;
        else
            this.closeIcon = embedIcons.closeDark;

        this.floaterIcon = embedIcons[this.settings.floaterIcon] || this.settings.floaterIcon;
        const searchIcon = embedIcons[this.settings.searchIcon] || this.settings.searchIcon;
...
class SearchBar extends HTMLElement {
    constructor() {
        ...
        this.showResults = (index = this.resultsIndex) => {
            ...
            if (queries.length > 1 || queriesContainMedia) {
                ...
                toolbar += `
					...
					<img class="jina-results-action-button${this.resultsView === 'list' ? ' jina-active' : ''}" src="${embedIcons.listView}" id="jina-toolbar-button-list" draggable="false">
					<img class="jina-results-action-button${this.resultsView === 'grid' ? ' jina-active' : ''}" src="${embedIcons.gridView}" id="jina-toolbar-button-grid" draggable="false">
				</div>`
            }
        ...
        this.originalSearchIcon = embedIcons[this.settings.searchIcon] || this.settings.searchIcon;
        this.innerHTML = `
